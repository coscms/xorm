...
func (condNotIn condNotIn) WriteTo(w Writer) error {
	...
	case []int8:
		...
		if _, err := fmt.Fprintf(w, "%s NOT IN (%s)", w.Key(condNotIn.col), questionMark[:len(questionMark)-1]); err != nil {
			return err
		}
	...
	case expr:
		...
		if _, err := fmt.Fprintf(w, "%s NOT IN (", w.Key(condNotIn.col)); err != nil {
			return err
		}
