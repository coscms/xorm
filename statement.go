...
func (statement *Statement) colName(col *core.Column, needTableName bool) string {
	return statement.buildColName(col, statement.TableAlias, needTableName)
}

func (statement *Statement) buildColName(col *core.Column, alias string, needTableName bool) string {
	var (
		name    string
		colName string
	)
...
func (statement *Statement) buildConds(table *core.Table, bean interface{}, includeVersion bool, includeUpdated bool, includeNil bool, includeAutoIncr bool, addedTableName bool) (builder.Cond, error) {
	...
	for _, col := range table.Columns() {
		...
		colName := statement.buildColName(col, statement.TableAlias, addedTableName)
...
func (statement *Statement) genColumns(table *core.Table, alias string, colNames *[]string, needTableName bool) {
	...
	for _, col := range table.MyColumns() {
		...
		name := statement.buildColName(col, alias, needTableName)
...
func (statement *Statement) genConds(bean interface{}) (string, []interface{}, error) {
	...
	return builder.ToSQL(statement.cond, statement.Engine.QuoteKey)
}
